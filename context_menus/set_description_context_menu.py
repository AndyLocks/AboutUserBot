...
async def descriptionContextMenu(interaction: discord.Interaction, message: discord.Message):


    if len(message.embeds) != 0:
        await interaction.response.send_message("The selected message cannot be selected as a description ❌", ephemeral=True)
        return
    ...
    emb = discord.Embed(
            title="**Attention!!!**",
            description=f"Are you sure that you want to change the description to:\n>>> {message.content}",
    ...
    async def result_yes(interaction: discord.Interaction):
        ...
        member.body_description = message.content
...
description_content_menu = app_commands.ContextMenu(
    name="set as descripton",
    callback=descriptionContextMenu
)
bot.tree.add_command(description_content_menu)